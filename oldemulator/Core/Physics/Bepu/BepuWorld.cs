...
namespace Eurosim.Core.Physics.BepuWrap
{
...
//				}
		...
		{			
			Box body = new Box(new Vector3(0, 0, 0),
				BepuConverter.ToSimUnits((float)xsize), BepuConverter.ToSimUnits((float)ysize), BepuConverter.ToSimUnits((float)zsize), 1);			
		...
		{			
			Cylinder body = new Cylinder(new Vector3(0, 0, 0),
				BepuConverter.ToSimUnits((float)height), BepuConverter.ToSimUnits((float)rbottom), 1);
