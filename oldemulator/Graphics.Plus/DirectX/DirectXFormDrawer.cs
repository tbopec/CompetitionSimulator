	...
	public partial class DirectXFormDrawer : FormDrawer, IDirectXDrawer
	{
		...
		private void SetUpAndWaitForRun(Func<DrawerSettings, Form> formFactory)
		{
			try
			{
				...
				Body robot = _scene.RootBody.GetSubtreeChildrenFirst().
					FirstOrDefault(x => x.Name != null && x.Name.Contains("Robot" + Settings.RobotNumber));
				_camera = new SwitchableCamera(robot, new Frame3D(0, 0, 20, Angle.FromGrad(-35), Angle.Zero, Angle.Zero),
