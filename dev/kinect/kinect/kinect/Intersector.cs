...
using eBall = Eurosim.Core.Ball;
using eBox = Eurosim.Core.Box;
using ePlane = AIRLab.Mathematics.Plane;
using eCylinder = Eurosim.Core.Cylinder;
...
namespace kinect
{
    public partial class Intersector
    {
       ...
       #region TriangleMethod
        internal static class TriangleIntersection
        {
            ...
            public static bool IsInsideTriangle(Point2D v0, Point2D v1, Point2D v2, Point2D p)
            {
                double f1 = (p.Y - v0.Y) * (v1.X - v0.X) - (p.X - v0.X) * (v1.Y - v0.Y);
                if (f1 <= 0)
                {
                    double f2 = (p.Y - v1.Y) * (v2.X - v1.X) - (p.X - v1.X) * (v2.Y - v1.Y);
                    if (f2 <= 0)
                    {
                        double f3 = (p.Y - v2.Y) * (v0.X - v2.X) - (p.X - v2.X) * (v0.Y - v2.Y);
                        if (f3 <= 0) return true;
                        else return false;
                    }
            ...
            public static Point3D? IntersectTriangle(Point3D p0, Point3D p1, Point3D p2, Ray ray)
            {
                ...
                else
                {
                    ...
                    return IsInsideTriangle(v0, v1, v2, point) ? point1 : null;
                }
            }
        }
    ...
    #region TriangleTest
    [TestFixture]
    internal class Triangle2DTest
    {

        private static readonly Point2D p0 = new Point2D(1, 0);
        private static readonly Point2D p1 = new Point2D(0, 1);
        private static readonly Point2D p2 = new Point2D(1, 1);
        private readonly object[][] _data5 = new []
            {
                new object[]
                    {
                        p0, p1, p2,
                        new Point2D(0.5, 0.5), //внутри
                        true
                    },
                new object[]
                    {
                        p0, p1, p2,
                        new Point2D(0.5, 1),//на стороне
                        true
                    },
                new object[]
                    {
                        p0, p1, p2,
                        new Point2D(1, 1),//в вершине
                        true
                    },
                new object[]
                    {
                        p0, p1, p2,
                        new Point2D(0, 0),//снаружи
                        false
                    }
            };
        [Test]
        [TestCaseSource("_data5")]
        public void RayTest(Point2D p0, Point2D p1, Point2D p2, Point2D p, bool type)
        {
            Assert.AreEqual(type, Intersector.TriangleIntersection.IsInsideTriangle(p0, p1, p2, p));
        }
    }
    [TestFixture]
