...
namespace AIRLab.Thornado
{
    partial class TypeResolver
    {
        static void InitImmutableConstructors()
        {
            ...
            ImmutableResolver<Point2D>.AddConstructor(new Func<double, double, Point2D>((X, Y) => new Point2D(X, Y)));

            ImmutableResolver<Point3D>.AddConstructor(new Func<double, double, double, Point3D>((X, Y, Z) => new Point3D(X, Y, Z)));
