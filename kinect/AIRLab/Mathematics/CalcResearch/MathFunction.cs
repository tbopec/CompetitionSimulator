...
using AIRLab.Mathematics.CalcResearch;
...
namespace AIRLab.Mathematics.CalcResearch
{
    public class SystemOfHomogeneousLinearEquations
    {
        ...
        {
            for (int currentX = 0; currentX < system.Length; currentX++)
            {
	...
	public class SimpleRegressionModule
	{
		...
		public  MathFunction GetOptimalFunction(string type)
		{
            if (type == "exp")
                return GetExp();
            if (type == "pol3")
...
public class Polynom : MathFunction
{
...
public class ExponentialFunctiuon:MathFunction
	{
	...
	public class NewRegressModuleTest
	{
		...
		public void TestExp()
		{
			...
			var optimalFunction = regressModule.GetOptimalFunction("exp");
		}
        [Test]
        public void LinerySystem()
        {
            var system = new double[][]
                                {
                                    new double[]{2,3,-1,4}, 
                                    new double[]{9,5,3,0}, 
                                    new double[]{4,0,-2,-10} 
            ...
            var optimalFunction = SystemOfHomogeneousLinearEquations.FindX(system);
        }
        [Test]
        public void TestPol()
        {
            ...
            var regressModule = new SimpleRegressionModule { Sample = sample };
            var optimalFunction = regressModule.GetOptimalFunction("pol3");
        }
	}
