...
namespace AIRLab.Mathematics
{
	public struct Triangle2D
	{
		...
		public Circle2D GetInscribedCircle()
		{
			...
			return new Circle2D(Angem.GetCrossing(GetBissectorFromA(), GetBissectorFromB()), GetInscribedCircleRadius());
		}

		private double GetInscribedCircleRadius()
		{
			double halfPerimeter = (LengthAB + LengthAC + LengthBC)/2;
			return Math.Sqrt((halfPerimeter - LengthAB)*(halfPerimeter - LengthAC)*(halfPerimeter - LengthBC)/halfPerimeter);
		}
...
//
//			Point2D finishBissector = Angem.GetCrossing(line, fakeBissector);
//			return new Line2D(startBissector, finishBissector);
			...
			var finishBissector = leftPoint*(1-alpha) + rightPoint*alpha;
